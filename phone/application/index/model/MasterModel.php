...
class MasterModel extends Model
{
   ...
   public function getcomment($param){
	 ...
	 //查找师傅的完工图片
   public function tupian($wid){
       $field = 'b.afterpics,c.img2,d.img as d_img,a.id,a.service_type_id'
       $tupian = Db::name('orders')->alias('a')
               ->join('master_reports b','a.worker_id=b.worker_id')
               ->join('install_tupian c','a.worker_id = c.worker_id')
               ->join('replace_report d','a.worker_id = d.worker_id')
               ->where('a.worker_id',$wid)->order('a.id desc')->limit(8)->select();
        return $tupian;
   }   
