    ...
    public function helper1($order,$wid)
    {
      ...
      $wa=  session("id");
        ...
        if ($results) {
            ...
            $this->assign('xuqiu',$resultsr['skill']);
            $this->assign('jine',$resultsr['cost']);
        ...
        if (empty($param['start'])) {
            ...
            $result=  Db::name("orders")->where('order_number',$order)->field("num")->find();
    ...
    public function helper3(){
     ...
     if(empty($red)){
         ...
         if($res){
             ...
             $order=$parme['order_number'];
             $link=$parme['link'];
             $worker_id=session("id");
             $api->add_messages(2,session("id"),$parme['worker_id'],"有师傅找你帮忙,请点击查看",$link."/$order/wid/$worker_id");
         }
     }else{
        if($red['status']==0){
...
//    删除帮手师父
    public function helper4(){
       ...
       if($red){
           ...
           $worker_id=$parme['worker_id'];
           $api->add_messages(2,session("id"),$parme['worker_id'],"主任师傅取消了你找帮手师傅","#");
       }else{
