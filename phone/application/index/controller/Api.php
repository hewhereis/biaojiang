...
//  服务类型
    ...
    public function brasn($in){
        $brand = Db::name('brand_list')->field('id as value,brand as title')->where('id','=',$in)->find();
        return $brand;
    }
    /**
      * 数组重组
...
public function users($id){
    $users = Db::name('users')->field('id as value,img as img,real_name')->where('id','=',$id)->find();
    return $users;
}
   public function submit_reports1(){
         ...
         * 发送消息
         */
        public function add_messages($type,$source_id,$receive_id,$content,$link){
