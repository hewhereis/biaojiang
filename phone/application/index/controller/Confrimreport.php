...
class Confrimreport extends Base
{
    /**
     * 师傅维修报告页面
     */
    public function master_reports($order_number){
         ...
         if(empty($arr)){
            ...
            $beforepics = array($update_images[0]['beforepics']);
            $afterpics = array($update_images[0]['afterpics']);
    ...
    public function submit_reports(){
      $param = input();
      $uid=session('id');
      $param['wid'] = session('id');
      $list = new ConfrimreportModel();
      $list = $list->submit_reports($param);
      //发送短信消息
       $type = 1;
       $source_id = $uid;
       $receive_id = Db::name('orders')->where('order_number',$param['order_number'])->value('owner_id');
       $content = '师傅已提交维修确认报告，请点击查看';
       $link = $param['link'];
       $api = new Api();
       $api = $api->add_messages($type,$source_id,$receive_id,$content,$link);
    ...
    public function confrim_reports(){
        ...
        $type = 1;
        $source_id = $uid;
        $receive_id = Db::name('orders')->where('order_number',$order_number)->value('worker_id');
        $content = '客户已确认维修确认报告，请点击查看';
        $link = $link;
        $api = new Api();
        $api = $api->add_messages($type,$source_id,$receive_id,$content,$link);
        //修改步骤
